          ...
          ifelse(country %in% c("Albania", "Armenia", "Azerbaijan", "Belarus", "Bosnia and Herzegovina", "Bulgaria", "Croatia", "Czech Republic", "Denmark", "Euro Area", "Georgia", "Hungary", "Iceland", "Kazakhstan", "Kyrgyz Republic", "Moldova", "North Macedonia", "Norway", "Poland", "Romania", "Serbia", "Sweden", "Switzerland", "Tajikistan", "Turkey", "Ukraine", "United Kingdom"), "Europe and Central Asia", 
           ifelse(country %in% c("Antigua and Barbuda", "Argentina", "Bahamas", "Barbados", "Belize", "Bolivia", "Brazil", "Chile", "Colombia", "Costa Rica", "Dominica", "Dominican Republic", "Grenada", "Guatemala", "Guyana", "Haiti", "Honduras", "Jamaica", "Mexico", "Nicaragua", "Panama", "Paraguay", "Peru", "St. Kitts and Nevis", "St. Lucia", "St. Vincent and the Grenadines", "Suriname", "Trinidad and Tobago", "Uruguay", "Venezuela, RB"), "Latin America and The Caribbean",
            ifelse(country %in% c("Algeria", "Djibouti", "Egypt", "Iraq", "Israel", "Jordan", "Kuwait", "Morocco", "Oman", "Qatar", "Syrian Arab Republic", "Tunisia", "United Arab Emirates"), "Middle East and North Africa", 
             ifelse(country %in% c("Canada", "United States", "Mexico"), "North America",
...
aggregate_cd <- ggplot(world_cd, aes(x=year, y=avg)) + geom_point() + geom_line(size = 1.25) + labs(title = "Change in Aggregate World Currency Demand 2001-2018", subtitle = "Significant Positive Trend", x= "Year", y = "Average Currency Demand", caption = "Source: IMF; BoE; SNB; RBI; BCRA") 
print(aggregate_cd)
...
region_graph <- ggplot(region_cd, aes(x=year, y=avg, color = region)) + geom_point() + geom_line() + labs(title = "Change in Aggregate Cash Demand by Region", subtitle = "2001 - 2018", x= "Year", y = "Average Currency Demand", color = "World Bank Regions", caption = "Source: IMF; BoE; SNB; RBI; BCRA")

#Remarkedly similar trends across regions
...
development_graph <- ggplot(development_cd, aes(x=year, y=avg, color = development)) + geom_point() + geom_line() + labs(title = "Change in Aggregate Cash Demand by Development", subtitle = "2001 - 2018", x= "Year", y = "Average Currency Demand", color = "World Bank Development", caption = "Source: IMF; BoE; SNB; RBI; BCRA")

ggsave("graphs.jpg", arrangeGrob(region_graph, development_graph, ncol = 2))

p <- plot_grid(region_graph, development_graph)
save_plot("plot2.jpg", p, ncol = 2)
...
east_asia_cd_pc <- east_asia_2001 %>%
  ...
  mutate(country = ifelse(country == "Papua New Guinea", "Papua N. Guinea", country)) %>%
  mutate(difference = ((`2018` - `2001`)/`2001`) * 100) %>%
  arrange(desc(difference)) %>%
  filter(country != "Brunei Darussalam")

east_asia_cd_pc_graph <- ggplot(east_asia_cd_pc, aes(x = reorder(country, difference), y = difference, fill =  reorder(country, -difference))) + geom_col() + coord_flip() + theme(legend.position = "none") + labs(title = "Percent Change in Cash Demand", subtitle = "East Asia and Pacific 2001 - 2018", caption = "Source: IMF - International Financial Statistics", y = "Percent Change", x = "Country") 
...
europe_cd_pc <- europe_2002 %>%
  ...
  mutate(country = ifelse(country == "Kyrgyz Republic", "Kyrgyz Rep.",
                   ifelse(country == "Czech Republic", "Czech Rep",
                   ifelse(country == "North Macedonia", "N. Macedonia",
                   ifelse(country == "Bosnia and Herzegovina", "Bosnia", 
                   ifelse(country == "United Kingdom", "UK", country))))))

europe_cd_pc_graph <- ggplot(europe_cd_pc, aes(x = reorder(country, difference), y = difference, fill =  reorder(country, -difference))) + geom_col() + coord_flip() + theme(legend.position = "none") + labs(title = "Percent Change in Cash Demand", subtitle = "Europe and Central Asia 2002 - 2018", caption = "Source: IMF - International Financial Statistics; BoE; SNB", y = "Percent Change", x = "Country") 
...
la_cd_pc <- la_2001 %>%
  ...
  mutate(country = ifelse(country == "St. Kitts and Nevis", "St. Kitts",
                    ifelse(country == "Trinidad and Tobago", "TT",
                     ifelse(country == "Antigua and Barbuda", "Antigua",
                     ifelse(country == "St. Vincent and the Grenadines", "St. Vincent", 
                     ifelse(country == "Dominican Republic", "DR", country))))))
                        

la_cd_pc_graph <- ggplot(la_cd_pc, aes(x = reorder(country, difference), y = difference, fill =  reorder(country, -difference))) + geom_col() + coord_flip() + theme(legend.position = "none") + labs(title = "Percent Change in Cash Demand", subtitle = "Latin America & The Caribbean 2001 - 2018", caption = "Source: IMF - International Financial Statistics; BCRA", y = "Percent Change", x = "Country") 

+ scale_y_continuous(breaks = c(-5, -2.5, 0, 2.5, 5, 7.5, 10, 12.5, 15, 17.5, 20)) 
...
me_cd_pc <- me_2002 %>%
  ...
  arrange(desc(difference)) %>%
  filter(!country %in% c("Jordan", "Iraq", "Qatar", "Kuwait"))

me_cd_pc_graph <- ggplot(me_cd_pc, aes(x = reorder(country, difference), y = difference, fill =  reorder(country, -difference))) + geom_col() + coord_flip() + theme(legend.position = "none") + labs(title = "Percent Change in Cash Demand", subtitle = "Middle East and North Africa 2002 - 2018", y = "Percent Change", x = "Country", caption = "Source: IMF - International Financial Statistics") 
...
ssa_cd_pc <- ssa_2001 %>%
  ...
  mutate(difference = ((`2018` - `2001`)/`2001`) * 100) %>%
  arrange(desc(difference)) %>%
  filter(!country %in% c("Zimbabwe", "Madagascar", "Liberia", "Angola", "Botswana", "Zambia", "Tanzania", "Rwanda", "Eswatini", "Seychelles")) %>%
  mutate(country = ifelse(country == "Central African Republic", "CAR",
                   ifelse(country == "Sao Tome and Principe", "STP",
                   ifelse(country == "Equatorial Guinea", "GNQ", country))))

ssa_cd_pc_graph <- ggplot(ssa_cd_pc, aes(x = reorder(country, difference), y = difference, fill =  reorder(country, -difference))) + geom_col() + coord_flip() + theme(legend.position = "none") + labs(title = "Percent Change in Cash Demand", subtitle = "Sub-Saharan Africa 2001 - 2018", y = "Percent Change", x = "Country", caption = "Source: IMF - International Financial Statistics") 
...
na_cd_pc_graph <- ggplot(na_cd_pc, aes(x = reorder(country, difference), y = difference, fill =  reorder(country, -difference))) + geom_col() + coord_flip() + theme(legend.position = "none") + labs(title = "Percent Change in Currency Demand in North America 2001 - 2018", subtitle = "Significant Increases in the Majority of Countries", y = "Percent Change", x = "Country", caption = "Source: IMF - International Financial Statistics") 
...
sa_cd_pc <- sa_2001 %>%
  right_join(sa_2018, by = "country") %>%
  mutate(difference = ((`2018` - `2001`)/`2001`) * 100) %>%
  arrange(desc(difference)) %>%
  filter(country != "Afghanistan")

sa_cd_pc_graph <- ggplot(sa_cd_pc, aes(x = reorder(country, difference), y = difference, fill =  reorder(country, -difference))) + geom_col() + coord_flip() + theme(legend.position = "none") + labs(title = "Percent Change in Cash Demand", subtitle = "South Asia 2001 - 2018", y = "Percent Change", x = "Country", caption = "Source: IMF - International Financial Statistics; RBI") 

g <- plot_grid(east_asia_cd_pc_graph, europe_cd_pc_graph, la_cd_pc_graph, me_cd_pc_graph, sa_cd_pc_graph, ssa_cd_pc_graph, labels = "AUTO", nrow = 3)
save_plot("percent_change.jpg", g, nrow = 3)
...
urbanization <- read_csv("raw-data/urbanization.csv", skip = 3) %>%
select(-`Indicator Name`, -`Indicator Code`) %>%
gather(year, value, -c(`Country Name`, `Country Code`)) %>%
filter(value != "NA") %>%
rename(country = `Country Name`) %>%
...
merged_2 <- cash %>%
  full_join(interest_rates_imf, by = c("country", "year")) %>%
  full_join(interest_rates_wdi, by = c("country", "country_code", "year")) %>%
  full_join(tax_gdp, by = c("country", "country_code", "year")) %>%
  full_join(eci, by = c("country", "year")) %>%
  full_join(uncertainty, by = c("country_code", "year")) %>%
  full_join(economic_freedom, by = c("country", "year")) %>%
  full_join(shadow_economy, by = c("country", "year")) %>%
  full_join(urbanization, by = c("country", "country_code", "year")) %>%
  full_join(crime, by = c("country", "country_code", "year")) %>%
  full_join(urban_pop_growth, by = c("country", "country_code", "year")) %>%
  mutate(region = ifelse(country %in% c("Australia", "Brunei Darussalam", "Cambodia", "Fiji", "Hong Kong", "Indonesia", "Japan", "Korea, Rep.", "Macao", "Malaysia", "Mongolia", "Myanmar", "Thailand", "Papua New Guinea", "Philippines", "Samoa", "Singapore", "Solomon Islands", "Tonga", "Vanuatu", "New Zealand"), "East Asia and Pacific",
          ifelse(country %in% c("Albania", "Armenia", "Azerbaijan", "Belarus", "Bosnia and Herzegovina", "Bulgaria", "Croatia", "Czech Republic", "Denmark", "Euro Area", "Georgia", "Hungary", "Iceland", "Kazakhstan", "Kyrgyz Republic", "Moldova", "North Macedonia", "Norway", "Poland", "Romania", "Serbia", "Sweden", "Switzerland", "Tajikistan", "Turkey", "Ukraine", "United Kingdom"), "Europe and Central Asia", 
           ifelse(country %in% c("Antigua and Barbuda", "Argentina", "Bahamas", "Barbados", "Belize", "Bolivia", "Brazil", "Chile", "Colombia", "Costa Rica", "Dominica", "Dominican Republic", "Grenada", "Guatemala", "Guyana", "Haiti", "Honduras", "Jamaica", "Mexico", "Nicaragua", "Panama", "Paraguay", "Peru", "St. Kitts and Nevis", "St. Lucia", "St. Vincent and the Grenadines", "Suriname", "Trinidad and Tobago", "Uruguay", "Venezuela, RB"), "Latin America and The Caribbean",
            ifelse(country %in% c("Algeria", "Djibouti", "Egypt", "Iraq", "Israel", "Jordan", "Kuwait", "Morocco", "Oman", "Qatar", "Syrian Arab Republic", "Tunisia", "United Arab Emirates"), "Middle East and North Africa", 
  ...
  mutate(cd_percent = cd *100) %>%
  mutate(tax_gdp_percent = tax_gdp* 100)
...
write.csv(merged_2, "merged_2.csv")

crime <- read_csv("raw-data/intentional_homocide.csv", skip = 3) %>%
select(-`Indicator Name`, -`Indicator Code`) %>%
gather(year, value, -c(`Country Name`, `Country Code`)) %>%
filter(value != "NA") %>%
rename(country = `Country Name`) %>%
...
regression_time_series <- plm(formula = cd ~ interest_rates_imf + tax_gdp + crime + urban_pop_growth, index = c("country", "year"), model = "within", effect = "twoway", data = time_series_merged_3)

regression_time_series_robust <- coeftest(regression_time_series, vcov=vcovHC(regression_time_series,type="HC1",cluster="group"))

stargazer(regression_time_series_robust,  title = "Regression Results - Cross-Country and Time Fixed Effects", type = "html", dep.var.labels= "cd", out = "final.html") 
...
tax_gdp_plot <- merged %>%
  ...
  summarize(tax_gdp = mean(tax_gdp, na.rm = TRUE))

ggplot(tax_gdp_plot, aes(x = year, y = tax_gdp, color = region)) + geom_point() + geom_line()

tax_gdp_plot_total <- merged %>%
  group_by(year) %>%
  summarize(tax_gdp = median(tax_gdp, na.rm = TRUE))

ggplot(tax_gdp_plot_total, aes(x = year, y = tax_gdp)) + geom_point() + geom_line()

plot <- merged %>%
  filter(cd_percent <= 100)

ggplot(plot, aes(x=tax_gdp, y=cd)) + geom_point() + geom_smooth(method = "lm", se = FALSE)

time_series <- read_csv("raw-data/long_time_series.csv") %>%
  group_by(country) %>%
  filter(!is.na(cd)) %>%
  filter(!country %in% c("Nigeria", "New Zeland"))

ggplot(time_series, aes(x=year, y=cd, color = country)) + geom_line(size =1.25) + labs(title = "Long Time Series of Cash Demand", subtitle = "Reversal of the Long Period of Decline in the 1990s", x = "Year", y = "Cash Demand (CiC / GDP)", color = "Country")
  

large_small_full_dataset <- large_small_cd %>%
  left_join(interest_rates_imf, by = c("country", "year")) %>%
  left_join(interest_rates_wdi, by = c("country", "country_code", "year")) %>%
  left_join(tax_gdp, by = c("country", "country_code", "year")) %>%
  left_join(eci_2, by = c("country", "year")) %>%
  left_join(uncertainty_2, by = c("country_code", "year")) %>%
  left_join(economic_freedom_2, by = c("country", "year")) %>%
  left_join(shadow_economy, by = c("country", "year")) %>%
  left_join(urbanization, by = c("country", "country_code", "year")) %>%
  left_join(crime, by = c("country", "country_code", "year")) %>%
  mutate(region = ifelse(country %in% c("Australia", "Brunei Darussalam", "Cambodia", "Fiji", "Hong Kong", "Indonesia", "Japan", "Korea, Rep.", "Macao", "Malaysia", "Mongolia", "Myanmar", "Thailand", "Papua New Guinea", "Philippines", "Samoa", "Singapore", "Solomon Islands", "Tonga", "Vanuatu", "New Zealand"), "East Asia and Pacific",
          ifelse(country %in% c("Albania", "Armenia", "Azerbaijan", "Belarus", "Bosnia and Herzegovina", "Bulgaria", "Croatia", "Czech Republic", "Denmark", "Euro Area", "Georgia", "Hungary", "Iceland", "Kazakhstan", "Kyrgyz Republic", "Moldova", "North Macedonia", "Norway", "Poland", "Romania", "Serbia", "Sweden", "Switzerland", "Tajikistan", "Turkey", "Ukraine", "United Kingdom"), "Europe and Central Asia", 
           ifelse(country %in% c("Antigua and Barbuda", "Argentina", "Bahamas", "Barbados", "Belize", "Bolivia", "Brazil", "Chile", "Colombia", "Costa Rica", "Dominica", "Dominican Republic", "Grenada", "Guatemala", "Guyana", "Haiti", "Honduras", "Jamaica", "Mexico", "Nicaragua", "Panama", "Paraguay", "Peru", "St. Kitts and Nevis", "St. Lucia", "St. Vincent and the Grenadines", "Suriname", "Trinidad and Tobago", "Uruguay", "Venezuela, RB"), "Latin America and The Caribbean",
            ifelse(country %in% c("Algeria", "Djibouti", "Egypt", "Iraq", "Israel", "Jordan", "Kuwait", "Morocco", "Oman", "Qatar", "Syrian Arab Republic", "Tunisia", "United Arab Emirates"), "Middle East and North Africa", 
...
write_csv(large_small_full_dataset, "raw-data/large_small_dataset.csv")
...
economic_freedom_2 <- economic_freedom_ea %>%
  full_join(economic_freedom, by = c("country", "year", "monetary_freedom", "business_freedom", "property_rights")) %>%
  arrange(country, year)

japan_imf_interest_rate <- read_csv("raw-data/ir01_m_1_en.csv") %>%
mutate(dec = str_detect(date, "/12")) %>%
  filter(dec == "TRUE")

  write_csv(japan_imf_interest_rate, "raw-data/ir01_m_1_en.csv")

time_series_2 <- read_xlsx("raw-data/long_time_series.xlsx") 

urban_pop_growth <- read_csv("raw-data/urban_pop_growth.csv", skip = 3) %>%
  select(-`Indicator Name`, -`Indicator Code`)%>%
gather(year, value, -c(`Country Name`, `Country Code`)) %>%
filter(value != "NA") %>%
rename(country = `Country Name`) %>%
...
interest_rates_imf_2 <- 
  
  time_series_tax_gdp <- time_series_2 %>%
  select(country, year, tax_gdp_2) %>%
  rename(tax_gdp = tax_gdp_2) %>%
  drop_na(tax_gdp) %>%
  full_join(tax_gdp, by = c("country", "year", "tax_gdp")) %>%
  select(-country_code) %>%
  group_by(country, year) %>%
  count()
  
time_series_imf_rates <- time_series_2 %>%
  select(country, year, bank_rate) %>%
  rename(interest_rates_imf = bank_rate) %>%
  drop_na(interest_rates_imf) %>%
  full_join(interest_rates_imf, by = c("country", "year", "interest_rates_imf")) %>%
  distinct()

time_series_debt_gdp <- time_series_2 %>%
  select(country, year, debt_gdp_2) %>%
  rename(debt_gdp = debt_gdp_2) %>%
  drop_na(debt_gdp) %>%
  full_join(debt_gdp, by = c("country", "year", "debt_gdp"))
  

time_series_merged_2 <- time_series_2 %>%
  left_join(interest_rates_imf, by = c("country", "year")) %>%
  left_join(tax_gdp, by = c("country", "year")) %>%
  left_join(urban_pop_growth, by = c("country", "year")) %>%
  left_join(crime, by = c("country", "year")) %>%
  select(-`country_code.y`, -`country_code.x`) 

write_csv(time_series_merged_2, "time_series_merged_2.csv")

time_series_merged_3 <- read_csv("time_series_merged_2.csv") %>%
  select(-bank_rate, -tax_gdp_2)
  
long_time_series_merged <- time_series_2 %>%
  left_join(time_series_tax_gdp, by = c("country", "year")) %>%
  select(-tax_gdp_2) %>%
  left_join(time_series_imf_rates, by = c("country", "year")) %>%
  select(-bank_rate) %>%
  left_join(urban_pop_growth, by = c("country", "year")) %>%
  left_join(crime, by = c("country", "year")) %>%
  select(-`country_code.y`, -`country_code.x`)  %>%
...
write_csv(long_time_series_merged, "time_series_merged.csv")
